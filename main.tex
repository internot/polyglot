...
\title{\Large \bf Polyglots : Crossing Origins by Crossing Formats}
...
\paragraph{Generalizing polyglots} 
...
from directly accessing the content handled by the plug-in, a number of plug-ins provide 
...
instances. For example, we show that GIFAR attacks describe a form of content smuggling, and
cross-origin CSS attacks describe a form of syntax injection attacks.
%
...
format, thereby creating a polyglot. The polyglot is benign when viewed as the 
original format, but malicious when viewed as the injected format. While the 
targeted service serves the content as its original format, it is in the hands 
of the attacker to decide how it is interpreted. Examples of 
such services include social networks, search engines, i.e., nearly any dynamic user driven website.
%
The targeted service is assumed to employ server-side cross-site scripting 
sanitization. Hence, for the attack to be successful, the input must 
bypass this sanitization. The sanitization filter will remove or encode 
problematic characters related to HTML-syntax, based on the context of the web application. In a polyglot attack
the injected format will pose as a new context, unknown to the filter. Chances are that the 
...
The victim in this scenario is an authenticated user of the targeted website, \emph{vulnerable.com}. 
The \emph{vulnerable.com} site serves the polyglot content as the benign content type, harmless to the victim. 
At a certain point, while still authenticated to \emph{vulnerable.com}, 
the victim unsuspectingly visits the attackers malicious website, \emph{attacker.com} (1).
The \emph{attacker.com} site serves a page that embeds the content from \emph{vulnerable.com} (2). 
The attacker supplies polyglot syntax in the input to the vulnerable 
service, e.g. in the parameters of the request for the embedded content (3).
The \emph{vulnerable.com} service responds with content of the benign content type (4), but
the way it is embedded in the attackers page coerces the browser to interpret it as the malicious content type.
The malicious payload is executed in the origin of \emph{vulnerable.com} and can 
hence leverage the credentials of the victim to further exploit the vulnerability.

\begin{figure}[t]
  \center
  \includegraphics[width=0.49\textwidth]{injection-scenario.png}
  \caption{Overview of the syntax injection scenario}
...
users submit files to the service which are subsequently served under the origin of 
...
accepts a limited set of file-formats, and the files are run through a filter to verify that they 
belong to this set, e.g. images, before being served to the end
user. The
service will verify the file under the assumption that it conforms strictly to one format. 
...
Since the attacker can coerce the content to be interpreted as the malicious format, the
service is tricked into serving malicious content. 
...
The victim in this scenario is an authenticated user of a targeted vulnerable website, \emph{vulnerable.com}. 
The target is infiltrated by the attacker (1), manipulating the website to
serve polyglot content. 
The \emph{vulnerable.com} site serves the polyglot content as the benign content type, harmless to the victim. 
At a certain point, while still authenticated to \emph{vulnerable.com}, 
the victim unsuspectingly visits the attackers malicious website, \emph{attacker.com} (2).
The \emph{attacker.com} site embeds the polyglot content from \emph{vulnerable.com} (3), but
coerces the browser to interpret it as the malicious content type (4). 
The capabilities of the malicious format determine the 
severity of the consequences of exploitation (5). 

To give more context to the scenario, \emph{vulnerable.com} can be a cloud storage service. 
The attacker infiltrates the target by uploading a polyglot to the service and embedding it 
on \emph{attacker.com}. While authenticated to \emph{vulnerable.com}, the unsuspecting victim visits \emph{attacker.com}, the embedded 
polyglot is activated and extracts all the files stored in the victims account.
...
\begin{figure}[t]
  ...
  \caption{Overview of the content smuggling scenario}
...
A stream is an unlimited sequence of bytes. A stream object shall always be indirect and consists of a 
...
parser. This is in part motivated by PDF being a container format, 
%of which the 
another design choice which implies
%implication is 
...
certain websites only escape these characters. This is sufficient to prevent cross-site 
...
The source code of the XML document reflects this response and can be retrieved at a later point. This 
results in a bidirectional communication channel.
%
As the response 
...
We refrain from mentioning names of individual websites to prevent exploitation 
before the issues have been properly dealt with. Neither do we want to disclose 
these websites as particularly vulnerable. We are in contact with the teams responsible for the websites to help
mitigate the vulnerabilities. 


\paragraph{Syntax injection}
The study is based on supplying the websites with the benign minimal PDF in 
...
injection based polyglot attack. If these tokens pass through unaltered, the website is 
considered vulnerable. Only public accessible input parameters, those that do not require authentication, 
were tested, which suggests that more websites are likely vulnerable in input that do
requires authentication.

The conclusion is 
that nine websites out of the hundred apply insufficient content filtering with 
respect to the PDF format. Out of the nine found to be vulnerable; three were susceptible to traditional 
XSS and thereby also to PDF polyglot attacks, two were susceptible only to PDF based polyglot attacks, and four applied 
insufficient content filtering, but the input was reflected in a way that prevented exploitation, e.g. the header appeared after 1024 bytes.
%
%, which have the potential of compromising the 
%confidentiality and integrity of the website's users.
%

The two web sites found susceptible only to polyglot attacks and not XSS attacks
are of particular interest. They both employ proper measures to protect properly 
...
were properly encoded, the backslash character was properly escaped, and the 
string "$<$/script$>$" was removed. None of these measures prevent an attacker 
from injecting valid PDF syntax.
\item The second website reflected the user input in an HTML-comment context. 
...
subsequently analyzed to determine which origin it was served under. One major 
enterprise cloud storage service was found to be susceptible to attacks. The service makes an effort to
follow current best practices, see Section~\ref{sec:smuggling-mitigation}, for almost all uploaded content; the exception being the user's 
avatar images that are served under the sensitive origin. An attacker could 
...
We are currently advising the service provider to help mitigate this vulnerability. 
...
affected components, both server-side and client-side. It should be noted that 
preventing polyglot attacks in the general case is a complex task, as one would 
...
Note that although some mitigation discussed below is already in place
...
injection apply to all services that rely on user input, and the content smuggling
...
To prevent PDF-based syntax injection attack, the task is simpler. As discussed in Section~\ref{sec:pdf-injection}, the token-set identified in 
...
These recommendations comes with a caveat that need to be taken into consideration. Some user supplied 
content is only meant to be accessible to certain authenticated users, e.g. photos that 
is only shared with friends. In that case, the service needs to transfer the 
...
of the \emph{object} tag. The vulnerability can be mitigated by acknowledging that there can be a potential security issue in 
...
will reduce the attack surface. However, it is not sufficient for complete 
mitigation. As discussed in Section~\ref{sec:pdf-based-polyglots} the PDF format is a container format; designed to 
