...
Polyglots allow
multiple interpretation of the content, providing a new space of attack vectors.
An attacker can use a polyglot to infiltrate a vulnerable origin.
Once infiltrated, the polyglot is embedded in the attackers web site using a plug-in.
According to the SOP, content loaded in a plug-in is considered to belong to the 
origin from which it was loaded, thus allowing the polyglot to communicate with the vulnerable 
origin. A victim visiting the attacker's web site, while authenticated in the vulnerable
origin, will activate the malicious polyglot, which can abuse the credentials of the victim 
...
attacks. Five out of the top 100 Alexa web sites are vulnerable to syntax injec
attacks. In addition,
we have found two major
enterprise cloud storage services to be susceptible to content
smuggling attacks.
...
Another example of problematic elements include the \emph{object} and
...
fail to cover scenarios for content upload. 
