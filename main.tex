...
been by researchers. Two noteworthy examples are GIFAR~\cite{XXX} and
cross-origin CSS attacks~\cite{YYY} (Section~\ref{sec:related}
discusses further realted work). 
%
...
exploitation: syntax injection and content smuggling.
...
\todo{discuss the layout}

\subsection{Infiltrating the origin}
...
A form of content repurposing attack.

\subsection{Exploiting the infiltrated origin}

After infiltrating an origin two lines of exploitation is available.

\subsubsection{Cross-domain leakage}

Extract sensitive information and send to attacker

\subsubsection{Cross-site request forgery}

Extract CSRF protection token and do CSRF attack, despite protection!
...
\todo{diagram with attack scenario. for example: with example domains attacker.com and
  vulnerable.com leaking secret file secret.txt}
% Description of attack technique 
A content smuggling attack targets the users of a website
...
Based on the minimal PDF sample in Listing~\ref{lst:minimal-pdf}, we can derive 
the set of tokens required to build a PDF. Assuming that alpha-numeric characters pass through any filter unmodified, this set is ${\%, <<, >>, (, ), /}$.
...
\newpage
\section*{Todo}
Test page \url{http://internot.noads.biz} with Billy's and Jonas' anonymized demos?
