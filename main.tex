...
\subsection{The object- and embed-tags}
\todo{consider renaming to "Embedded PDF documents", and describe the plug-in behaviour, origin of content, type handling, cookies?}
%The object-tag allows the developer to specify which format 
%the embedded content should be interpreted as.

\todo{consider moving to section "Crossing origins"}
The object- and embed-tags are used to embed content, which requires a browser plug-in
...
\subsection{PDF communication primitives}
...
Each requiring its own communication primitives.

When embedded in a web page, many of the PDF communication primitives utilize
the underlying browser to make requests by navigating the browser to the target 
URL. This will unload the current page in which the document is embedded. Such 
primitives can be used in Cross-Site Request Forgery (CSRF) attacks, but would 
defeat the purpose of requesting information as the document cannot process the 
response.

\subsubsection{The target}
There are two primitives for a PDF to communicate with the server it originated from, 
that allow it to read the response; from JavaScript using XML external entities 
or from an embedded flash file.
...
is included in the document, it has to be valid, well-formed XML. This puts a 
restriction on the content that can be requested using this method. Considering 
that HTML pages are rarely well-formed this method may be too restricted.
...
via the embedding page or through the attackers server. While it is not To prevent the document from unloading 
...
the hostContainer.postMessage method.

\paragraph{Hash transport}

\paragraph{Embedded Flash}
Cross-origin communication in Flash is restricted by Cross-Origin Resource Sharing. 
