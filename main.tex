...
it is manipulated by web applications. The different trust domains
...
restricts only allows access between two documents if they share the
...
The OWASP top 10 list~\cite{OWASP:top10:10} places injection
...
developers~\cite{XSS:understood}. Known defense include various
flavors of \emph{sanitization} on server side and the
\emph{content security policy} (CSP) on client side. Sanitization
is often performed by the server which sanitizes input data
...
\todo{Emphasize novel attack vectors}
...
This is not to be confused with Cross-Origin Resource Sharing (CORS)~\cite{CORS}, which is an 
...
navigation is allowed wtih respect to the SOP, since the response is not visible 
...
There are two methods for a PDF to communicate with the server it originated from, 
...
but also support cross-origin communication via Cross-Origin Resource Sharing 
(CORS). Using CORS, a web server can relax the SOP to allow access to specified content.
