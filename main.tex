...
\todo{More?} Typically a document is considered to be an HTML-page.
...
\todo{Describe general scenario with forward pointer}
...
measures. Section~\ref{sec:eval} shows that even state-of-the-art plug-ins fail to properly do so. As we will show, this can lead to 
...
In recent versions of Adobe Reader certain measures has been taken to prevent 
...
a match is found, the parser will abort loading of the document. The problem of 
...
counter example is the signature used for the JPG format. While the signature is 
...
"Adobe is default" it indicates that Adobe Reader is the commonly default PDF reader.
...
users, the results suggests that more websites are likely vulnerable in input that do
...
subsequently analyzed to determine which origin it was served under. Two major 
enterprise cloud storage services were found to be susceptible to attacks. Both service makes an effort to
follow current best practices, see Section~\ref{sec:smuggling-mitigation}, but fail to cover scenarios for content upload. 
