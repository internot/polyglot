...
This paper identifies two vectors for infiltrating an origin, via syntax 
injection, or content smuggling. 

\subsubsection{Syntax injection}

In a cross-site scripting attack, an HTML document is composed from input 
...
users to submit files to the service which are subsequently served under the origin of 
...
The scenario, illustrated in Figure~\ref{fig:scenario}, describe how the attack proceed, from the infiltration of the origin, to the compromise of the victim.
...
The attacker infiltrates the target by uploading a polyglot to the service and embed it 
...
parser. This is in part motivated by PDF being a container format, of which the 
...
supplied by the server is inappropriate for the plug-in.
