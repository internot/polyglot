...
conceptually and by a proof-of-concept web pages.
...
of the attacker to decide how it is interpreted.
%
...
\paragraph{Scenario}
The scenario, illustrated in Figure~\ref{fig:injection-scenario}, describe how 
the attack proceed, from the infiltration of the origin, to the compromise of the victim.
...
the service. Examples of such potentially vulnerable services are file sharing services, image databases, 
...
submission systems, which by design allow submission of PDFs to be viewed by
users.
...
src="vulnerable.com/content?input=%PDF...">
\end{lstlisting}
...
These requests allow for the extraction of sensitive user data that can either be communicated back to the attacker, or be leveraged in further attacks.
...
\subsection{Content smuggling}
\todo{Deal with this section, extend and polish}
...
is required. PDF documents provide multiple ways of generating HTTP requests. 
Many of these are cross-origin, but, out of security concerns, only one-way in the sense that 
the PDF document will never see the result. Therefore, the focus is directed towards 
two methods that allow bidirectional communication. Since the document will retain the 
...
getURL('attacker.com/?'+doc.saveXML())
endstream
trailer<</Root
   <</Pages<<>>
      /OpenAction
...
We have conducted a
study to evaluate the prevalence of the problem on popular sites, 
...
these websites as particularly vulnerable.
...
insufficient content filtering, but the input was reflected in a way that prevented exploitation.
%
%, which have the potential of compromising the 
%confidentiality and integrity of the website's users.
%
We are in contact with the teams responsible for the websites to help
...
affected components, from server to client.
...
To prevent PDF-based syntax injection attack, the task is simpler. The token-set identified in 
...
many cases. These best-practices successfully prevents content-smuggling 
attacks in all aspects, as the infiltrated origin is separate from the origin 
containing sensitive data. 

\todo{What about serving as attachment, so that it is not viewed within the browser? How does it work with embedded content?}
